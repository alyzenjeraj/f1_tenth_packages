...
alias ws-build="docker build -t f1tenth_ros2 -f docker/Dockerfile ."

# Function to run the Docker container interactively with a mounted volume
function ws-start {
    local workspace_dir=$(pwd)
    ROS2_CONTAINER_ID=$(docker run -d -it --runtime=nvidia --gpus all -v $workspace_dir/ros2_ws:/ros2_ws f1tenth_ros2)
